...
# UI
@export var DriftBoostReadyLabel : Label
@export var DriftBoostReadyLabelTimer : Timer

# Lap Tracking
@export var LapLabel : Label
...
func _ready():
	AnimatedSprite.play("%s_idle" % character_name)
...
func _physics_process(delta):
	...
	if result.size() > 0:
		...
		if collider and collider.is_in_group("road"):
			is_on_ground = true
			last_floor_position = result.position
		else:
			is_on_ground = false
	else:
		is_on_ground = false
...
func RotateCar(delta):
	...
	if rotate_input < 0:
		AnimatedSprite.play("%s_turn_left" % character_name)
	elif rotate_input > 0:
		AnimatedSprite.play("%s_turn_right" % character_name)
	else:
		AnimatedSprite.play("%s_idle" % character_name)
